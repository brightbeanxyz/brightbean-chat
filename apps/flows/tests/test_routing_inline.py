...
class TestTheGates:
    ...
    def test_a_drained_bucket_enqueues(self, tenancy, connection):
        ...
        has_send_capacity(connection)  # creates the row
        SendBucket.objects.filter(connection=connection).update(tokens=0, refilled_at=timezone.now())

        assert may_run_inline(connection, InlineBudget.start(), first_step_safe=True) is InlineDecision.NO_SEND_CAPACITY

    def test_the_capacity_probe_does_not_spend_a_token(self, tenancy, connection):
        """send_outbound performs the one real debit. A debiting gate here would
        charge every inline send twice and halve effective throughput."""
        from apps.messaging.models import SendBucket

        has_send_capacity(connection)
        SendBucket.objects.filter(connection=connection).update(tokens=5, refilled_at=timezone.now())

        for _ in range(3):
            assert has_send_capacity(connection) is True

        # Refill over the elapsed microseconds can only add, so anything at or
        # above the starting count proves nothing was debited.
        assert SendBucket.objects.get(connection=connection).tokens >= 5
