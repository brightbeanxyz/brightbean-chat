...
| **[L1-A](https://github.com/brightbeanxyz/brightbean-chat/issues/31)** Tenancy & auth | L0 | org/workspace + two-tier membership, two-table RBAC (org owner/admin/member + workspace admin/editor/agent/viewer), **org-level member management and invitations**, allauth, credential-resolution chain + override UI (the override table and UI were later removed; credentials are env-first), workspace-scoped base manager, IDOR fuzz helper, auth rate limiting |
