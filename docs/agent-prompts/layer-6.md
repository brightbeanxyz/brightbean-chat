...
One trap in that layer, added by #77 after the rest of this file was written. A `media` block holds no URL — it holds the platform's identifier, because fetching it needs that connection's credentials — so `rendering.py` puts a link to `apps.inbox.views.media`, our own workspace-scoped proxy route, in the view model. `is_renderable_url` is deliberately **not** consulted for that one, and its docstring says it "must not be": the function answers a question about somebody else's string, and this address is one we minted. Adding the check there for tidiness would break every inbound attachment. Every *other* URL still goes through it.

`apps/inbox/` today is still small in the ways that matter — `ConversationRead` is the only model and `mark_read` the only service. `apps/inbox/rendering.py` returns view models and **never HTML**, with an AST scan that keeps `mark_safe` out of the package, and URLs pass `apps.common.validators.is_renderable_url` before becoming an `href` or `src`. Labels and rule descriptions are user-authored text on that same attacker-content → team-browser path (SECURITY-BASELINE §2). Extend the rendering layer; do not route around it.
...
- Rendering is the security surface and it is enforced. apps/inbox/rendering.py returns view models and never HTML; an AST scan keeps mark_safe out of the package; URLs pass apps.common.validators.is_renderable_url before becoming an href or src; anything unshowable becomes a visible tombstone.
- ONE EXCEPTION, added by #77: a `media` block carries the platform's identifier rather than a URL, so rendering.py links to apps.inbox.views.media — our own workspace-scoped proxy. is_renderable_url is deliberately NOT consulted for that path and its docstring says it "must not be", because the function answers a question about somebody else's string and this address is one we minted. Adding the check there would break every inbound attachment. Every other URL still goes through it. Labels and rule descriptions are user-authored text on the same attacker-content → team-browser path (SECURITY-BASELINE §2). Extend that layer; do not route around it.
