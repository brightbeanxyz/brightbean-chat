...
#: set, which is not the same as the *emitted* set: ``broadcast.finished`` has
#: no emitter until L6-B, and subscribing to it today is a no-op rather than an
#: error. Keeping the list here rather than deriving it from
#: :func:`discover_catalog` means the settings UI offers a stable set of
#: checkboxes that does not change shape depending on which apps happen to be
#: installed.
#:
