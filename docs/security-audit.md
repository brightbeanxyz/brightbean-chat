...
| §6.5 | The two non-HTTP egress paths are named and bounded | `tests/test_ssrf_call_sites.py::TestEveryRequestLeavesThroughAKnownDoor` | COVERED — pinned so a third cannot appear quietly, and both now go through the shared classifier. SMTP is address-pinned; boto3 is address-checked on every request. The residual boto3 rebinding window is stated in §6.1 rather than filed, because it is a property of botocore rather than a gap in this codebase. |
